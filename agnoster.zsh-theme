...
prompt_status() {
  ...
  if [[ $AGNOSTER_STATUS_RETVAL_NUMERIC == 'true' ]]; then
    ...
    [[ $RETVAL -ne 0 ]] && symbols+="%{%F{$AGNOSTER_STATUS_RETVAL_FG}%} BEOK :( \n"
  fi
...
prompt_custom_status() {
  ...
  if [[ $AGNOSTER_STATUS_RETVAL_NUMERIC == 'true' ]]; then
    [[ $RETVAL -ne 0 ]] && symbols+="%F{$AGNOSTER_CONTEXT_BG}$RETVAL%f"
  else
    [[ $RETVAL -ne 0 ]] && symbols+="%F{$AGNOSTER_CONTEXT_BG} BEOK :( %f"
  fi

  [[ $UID -eq 0 ]] && symbols+="%F{$AGNOSTER_STATUS_ROOT_FG}⚡%f"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%F{$AGNOSTER_STATUS_JOB_FG}⚙%f"
  

  if [[ -n "$symbols" ]]; then
    # Use custom glyphs for rounded segment, optional
    local left_sep=$'\uE0B6'  # Left rounded
    local right_sep=$'\uE0B4' # Right rounded
    local bg="$AGNOSTER_STATUS_RETVAL_FG"
    local fg="$AGNOSTER_STATUS_RETVAL_FG"

    print -n "%F{$bg}$left_sep%f%K{$bg}%F{$fg} ${symbols} %f%k%F{$bg}$right_sep \n%f"
  fi
}
...
build_prompt() {
  ...
  prompt_custom_status
  CURRENT_BG=NONE
  prompt_dir_color="#d7d7d7" # Change this to dynamically match `prompt_dir` background color
  echo -n "%{%F{$prompt_dir_color}%}$SEGMENT_SEPARATOR0"

  #prompt_segment $prompt_dir_color $CURRENT_FG "󰣇" # Example character: ☀ 
  
  # Randomly select between logo_1 and logo_2
  if (( RANDOM % 2 )); then
    prompt_segment $prompt_dir_color $CURRENT_FG "$logo_1" # Use logo_1
  else
    prompt_segment $prompt_dir_color $CURRENT_FG "$logo_2" # Use logo_2
  fi
  prompt_virtualenv
  prompt_aws
  prompt_terraform
